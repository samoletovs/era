...
export function BankRecon() {
  ...
  const [accounts, setAccounts] = useState<
    Array<{ code: string; name: string; isPostable: boolean }>
  >([]);
  ...
  function refreshSelected() {
    if (!selected) return;
    api
      .bankReconciliation(companyId, selected.id)
  ...
  if (!companyId)
    return (
      <div className="empty-state">
        ...
        <p>Add a company first to manage bank reconciliation.</p>
      </div>
    );
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <div className="icon">🏦</div>
          <h3>No reconciliations</h3>
          <p>Import a bank statement to start reconciling.</p>
        </div>
      ) : (
        <UniversalGrid
          rows={recons}
