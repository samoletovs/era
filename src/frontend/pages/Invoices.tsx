...
export function Invoices() {
  ...
  function loadInvoices() {
    setLoading(true);
    api
      .invoices(companyId, filter || undefined)
      .then((data: any) => {
        setInvoices(data);
        setLoading(false);
      })
      ...
      if (creditCorrect) {
        try {
          ...
          if (fields?.lines?.length > 0) {
            ...
            try {
              await api.postInvoice(companyId, corrected.id);
            } catch {
              /* ok */
            }
          }
        } catch {
          /* corrected invoice creation is best-effort */
        }
      }
  ...
  async function handleCreateInvoice() {
    ...
    try {
      ...
      // Auto-post
      try {
        await api.postInvoice(companyId, invoice.id);
      } catch {
        /* draft is fine */
      }
      setActivePanel('');
  ...
  if (!companyId)
    return (
      <EmptyState
        ...
        description="Use the agent chat to create a company first."
      />
    );
      ...
      render: (inv) =>
        inv.status === 'posted' || inv.status === 'overdue' ? (
          <button
            ...
            aria-label={`Pay ${inv.invoiceNumber}`}
          >
            Pay
          </button>
        ) : null,
            ...
            <div className="settings-card">
              <div className="onboarding-details">
                <div className="detail-row">
                  <span className="detail-label">ERA number</span>
                  <span className="mono">{selected.invoiceNumber}</span>
                </div>
                {selected.vendorInvoiceNumber && (
                  <div className="detail-row">
                    <span className="detail-label">Vendor invoice #</span>
                    <span className="mono">{selected.vendorInvoiceNumber}</span>
                  </div>
                )}
                <div className="detail-row">
                ...
                <div className="detail-row">
                  ...
                  <span>{formatMoney(selected.subtotal, fmt)}</span>
                </div>
                <div className="detail-row">
                  <span className="detail-label">VAT</span>
                  <span>{formatMoney(selected.vatAmount, fmt)}</span>
                </div>
                <div className="detail-row" style={{ fontWeight: 600 }}>
                  <span className="detail-label">Total</span>
                  <span>{formatMoney(selected.total, fmt)}</span>
                </div>
                <div className="detail-row">
                  <span className="detail-label">Paid</span>
                  <span>{formatMoney(selected.amountPaid, fmt)}</span>
                </div>
                <div className="detail-row">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="settings-card" style={{ marginTop: 16 }}>
              ...
              <GlPostings
                ...
                emptyMessage="No GL entries (invoice not yet posted)"
            ...
            {creditNoteInv?.id === selected.id && (
              <div className="settings-card" style={{ marginTop: 16 }}>
                <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>
                  Credit note for {creditNoteInv.invoiceNumber}
                </h3>
                ...
                >
                  <input
                    type="checkbox"
                    checked={creditCorrect}
                    onChange={(e) => setCreditCorrect(e.target.checked)}
                  />
    ...
    <div>
            ...
            <div>
              ...
              >
                ...
                <div style={{ gridColumn: 'span 2' }}>
                  ...
                  <input
                    type="text"
                    value={parsedInvoice.contactName}
                    onChange={(e) =>
                      setParsedInvoice((p: any) => ({
                        ...p,
            ...
            {uploadStatus === 'processing' ? (
              <div className="drop-zone-content">
                ...
                <div className="drop-zone-subtitle">
                  GPT-4o is extracting data from your document
                </div>
              </div>
            ) : (
              <div className="drop-zone-content">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Create an invoice, upload a document, or use the agent chat."
        />
      ) : (
        <UniversalGrid
